...
function readRecentLogs(maxLines = 80) {
  const logs = [];
  const candidates = [
    path.join(__dirname, 'logs', 'monitor.log'),
    path.join(__dirname, 'logs', 'monitor.log.bak'),
    path.join(__dirname, 'logs', 'verification-loop.log'),
    path.join(__dirname, 'logs', 'deploy-agent.log'),
    path.join(__dirname, 'logs', 'approval.log'),
  ];
  for (const file of candidates) {
    ...
    try {
      ...
      for (let index = lines.length - 1; index >= 0 && logs.length < maxLines; index--) {
        const parsed = parseLogLine(lines[index]);
        if (parsed) logs.push(parsed);
      }
    } catch {
      // ignore read errors so dashboard remains available
    }
  }
...
app.get('/api/dashboard', (req, res) => {
  ...
  const endpointSummaries = [
    ...
    {
      endpoint: '/api/rubricas',
      ok: Object.keys(RUBRICS).length > 0,
      status: Object.keys(RUBRICS).length > 0 ? 200 : 500,
      durationMs: 0,
      error: Object.keys(RUBRICS).length ? null : 'No rubrics loaded',
      mode: 'derived',
    },
    {
      endpoint: '/api/evaluar',
      ok: typeof evaluateEssay === 'function',
      status: typeof evaluateEssay === 'function' ? 200 : 500,
      durationMs: 0,
      error: typeof evaluateEssay === 'function' ? null : 'Evaluator missing',
      mode: 'derived',
    },
  ];

  const rubricas = Object.keys(RUBRICS).map((id) => loadRubric(id)).filter(Boolean);
  const logs = readRecentLogs(60);
  ...
  const dashboard = {
    ...
    health: {
      ok: endpointSummaries.every((item) => item.ok),
    },
    rubricas,
...
module.exports = { app, evaluateEssay, loadRubric, RUBRICS };
